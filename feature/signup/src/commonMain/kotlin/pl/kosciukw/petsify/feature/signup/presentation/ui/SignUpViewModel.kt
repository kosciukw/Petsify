...
class SignUpViewModel(
    ...
    override fun onTriggerEvent(event: SignUpEvent) {
        when (event) {
            ...
            SignUpEvent.OnConfirmButtonClicked -> onConfirmButtonClicked()
            SignUpEvent.OnLoginButtonClicked -> setAction { SignUpAction.Navigation.NavigateToLogin }
            SignUpEvent.OnScreenDisposed -> clearForm()
        }
    }
    ...
    private fun onPasswordChanged(password: CharArray) {
        isPasswordValid = passwordValidator.isValid(password) && notEmptyValidator.isValid(password)
        ...
        when (matchState) {
            ...
            PasswordMatchState.Match -> {
                ...
                setState { copy(isConfirmPasswordValidationErrorEnabled = false) }
            }

            PasswordMatchState.Mismatch -> {
                isRepeatPasswordValid = false
                setState { copy(isConfirmPasswordValidationErrorEnabled = true) }
            }
        }
    ...
    private fun startOtpRegistration(email: String) {
        launch {
